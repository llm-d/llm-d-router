...
| `--tls-cipher-suites` | — | Go `crypto/tls` cipher suite names (comma-separated or repeated) | `crypto/tls` default | Set the TLS cipher suites accepted by the sidecar's secure serving listener. Only effective for TLS 1.2 and below; TLS 1.3 cipher suites are not configurable. |
