...
|---|---|---|---|---|
| `--secure-serving` | — | `true` / `false` | `true` | Serve the sidecar data-plane listener over TLS. The deprecated `--secure-proxy` alias remains supported. |
...
| `--tls-min-version` | — | `VersionTLS10`, `VersionTLS11`, `VersionTLS12`, `VersionTLS13` | `VersionTLS12` | Set the minimum TLS version accepted by the sidecar's secure serving listener. |
| `--tls-cipher-suites` | — | Go `crypto/tls` cipher suite names (comma-separated or repeated) | existing secure suite set | Set the TLS cipher suites accepted by the sidecar's secure serving listener. Only effective for TLS 1.2 and below; TLS 1.3 cipher suites are not configurable. |
