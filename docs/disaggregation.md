...
Deployments that do not declare any conditional-decode gate plugin still reject `Prefer: if-available` requests: after the PreRequest phase, the director rejects unclaimed conditional-decode requests with 412 by default so a missing gate plugin surfaces as the coordinator's cache-miss fallback rather than a silent forward.
