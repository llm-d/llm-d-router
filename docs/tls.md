...
| `--tls-min-version` | `VersionTLS10`, `VersionTLS11`, `VersionTLS12`, `VersionTLS13` | `VersionTLS12` | yes | yes | yes | Minimum TLS version for secure serving. |
...
Each component logs its TLS state once at startup, whether or not TLS is enabled:

```
server TLS  tls=true  cert_path=/etc/tls
```

The line reads `tls=false` when secure serving is disabled, and `cert_path` is empty
when no certificate directory is configured.
