...
It requires a `session-id-producer` plugin to extract the session identifier from the request,
and one or both of the affinity plugins below.

##### session-id-producer

Extracts a session identifier from an incoming request and publishes it as a request attribute
for affinity plugins to consume. Exactly one of `headerName` or `cookieName` must be set.

- **Type**: `session-id-producer`
- **Parameters**:
  - `headerName`: name of the request header to read the session ID from.
  - `cookieName`: name of the cookie to read the session ID from.

##### session-affinity-filter

Enforces hard session affinity: when a session ID is present and maps to a candidate endpoint,
only that endpoint is returned. When no session is present or the endpoint is not in the
candidate list, all endpoints pass through unchanged.

- **Type**: `session-affinity-filter`
- **Parameters**:
  - `sessionIDProducerName`: name of the `session-id-producer` instance to consume. Defaults to
    the built-in default producer when omitted.

##### session-affinity-scorer

Scores candidate pods by giving a high score to the pod used in previous requests in the same
session, and sets an HTTP cookie on the response to carry the session binding to the client.

- **Type**: `session-affinity-scorer`
- **Parameters**:
  - `maxAge`: cookie expiration time in seconds. Default `0` means a session cookie (expires when
    the browser closes).
  - `sessionIDProducerName`: name of the `session-id-producer` instance to consume. Defaults to
    the built-in default producer when omitted.

**Example** — hard affinity (filter) with cookie-based session tracking:
