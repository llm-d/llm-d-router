...
|------|-------|----------|----------------------------------|
| Serial WRITE (default) | `--moriio-write-mode` | Prefill first, await its response, then decode | **Propagated** from the prefill request's returned `remote_dp_rank` (the rank prefill actually ran on); falls back to a stable hash only if the response omits it |
