...
func (c *Client) Request(ctx context.Context, method, path string, body []byte, headers map[string]string) (*http.Response, error) {
	...
	if body != nil {
		if v := logger.V(logutil.TRACE); v.Enabled() {
			v.Info("request body", "method", method, "path", path, "headers", httplog.RedactedHeaders(req.Header), "http_body", RedactBody(body))
		}
	}
	...
	if v := logger.V(logutil.TRACE); v.Enabled() {
		...
		v.Info("response body", "status", resp.StatusCode, "http_body", RedactBody(respBody))
