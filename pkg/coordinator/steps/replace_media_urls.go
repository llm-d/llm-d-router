...
func (s *ReplaceMediaURLsStep) Execute(ctx context.Context, reqCtx *pipeline.RequestContext) error {
	...
	for msgIdx, msg := range messages {
		...
		for partIdx, part := range content {
			...
			if !ok {
				// Info, not DEBUG: the part is passed through untouched and the
				// request still succeeds, so this line is the only signal that
				// a client's media part was not downloaded. Logged at V(0) so
				// it shows at any verbosity.
