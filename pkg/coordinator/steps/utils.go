...
// collapsing to APITypeVLLMGenerate when useOpenAIFormat is false (the
// tokens-in format, whose body carries the prompt as reqCtx.TokenIDs and does
// not depend on the client's request shape); every other detected type passes
