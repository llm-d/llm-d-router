...
// Register's k8s wiring (build reader, register a boot-validation runnable)
// needs a real controller-runtime Manager, exercised in the integration
// tests via envtest. Here we cover Register's pure branches (disabled
// short-circuit + config validation) and validateRolesObserved by feeding
// a Controller wired to a fake reader.
...
func TestValidateRolesObserved_HappyPath(t *testing.T) {
	...
	if err := validateRolesObserved(context.Background(), c, []string{"prefill", "decode"}); err != nil {
...
func TestValidateRolesObserved_FailsIfNoRevisions(t *testing.T) {
	c := newTestController(validConfig())
	err := validateRolesObserved(context.Background(), c, []string{"prefill", "decode"})
...
func TestValidateRolesObserved_FailsIfRoleNeverObserved(t *testing.T) {
	c := newTestController(validConfig(), readyPod("p1", "v1", "prefill"))
	err := validateRolesObserved(context.Background(), c, []string{"prefill", "decode"})
