...
const (
	...
	ResponderExtensionPoint         = "Responder"
...
// Responder answers a request before it is matched to a model server. Responders run in
// configuration order and the first non-nil response wins.
type Responder interface {
	plugin.Plugin
	// Respond returns a non-nil LocalResponse to answer the request directly; the request
	// is never routed and no endpoint is selected. Return nil to defer to the next
	// Responder, and to normal routing once all decline. A non-nil error fails the request
