...
// Video and audio are populated today; image parsing slots in here later.
...
// audioMetadata. A missing or malformed value leaves the field zero so the
// estimator falls back to the payload, then config, then defaults.
...
func (b estimateBackend) appendChatMessage(out []byte, features []fwkrh.MultiModalFeature, msg fwkrh.Message, meta mmMetadata) ([]byte, []fwkrh.MultiModalFeature) {
	...
	for _, block := range msg.Content.Structured {
		...
		case blockTypeInputAudio, blockTypeAudioURL:
			data := block.InputAudio.Data + block.InputAudio.Format
			out, features = appendMMAsset(out, features, fwkrh.ModalityAudio, data, b.aud.placeholderCount(block.InputAudio, meta.audio))
		}
	}
	return out, features
}
