...
type tokenizerPluginConfig struct {
	...
	// Estimate configures the tokenizer-free byte-packing backend.
	Estimate *estimateConfig `json:"estimate,omitempty"`
	// ModelName is the model sent to the vLLM render endpoint.
...
func NewPlugin(ctx context.Context, name string, config *tokenizerPluginConfig) (*Plugin, error) {
	...
	if config.VLLM != nil {
		renderer, err := newVLLMHTTPRenderer(config.VLLM, config.ModelName)
		if err != nil {
