...
| `session-prefix-cache-producer` | [`sessionprefixcache`](sessionprefixcache/) | `PrefixCacheMatchInfo` | Resolves a session producer's engine-block prefixes against residency observed from vLLM KV-events with real or estimated tokens; requires a `SessionCacheRequest` producer upstream. |
...
- `inflight-load-producer` **optionally** consumes `PrefixCacheMatchInfo` from the configured prefix producer; prefix-discounting is applied automatically when the attribute is present.
