...
var (
	...
	llmdRequestTTFT = prometheus.NewHistogramVec(
		...
		[]string{"plugin_name", "plugin_type", "model_name", "target_model_name", "fairness_id", "objective"},
	)
...
func recordRequestTTFT(ctx context.Context, pluginName, pluginType, modelName, targetModelName, fairnessID, objective string, ttft float64) bool {
	if ttft < 0 {
	...
	llmdRequestTTFT.WithLabelValues(pluginName, pluginType, modelName, targetModelName, fairnessID, objective).Observe(ttft)
