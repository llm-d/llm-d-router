...
func TestProduce_RepairMatchExcludesSpeculativeOnlyBlocks(t *testing.T) {
	...
	idx := &fakeKVCacheIndexer{
		...
		matchBlockKeys: func(_ context.Context, _ []kvblock.BlockHash, _ sets.Set[string]) (map[string]kvcache.PodMatch, error) {
			return map[string]kvcache.PodMatch{pod: {MatchedBlocks: 3, ConfirmedBlocks: 2, WeightedScore: 3, BlocksByTier: map[string]int{"gpu": 1}}}, nil
		},
	}
	p := newProducerWithIndexer(ctx, idx)
	p.fullReportRepair = newTestRepair(0)
	req := &scheduling.InferenceRequest{
		RequestID:   "repair-counts",
		TargetModel: "model",
		Body: &fwkrh.InferenceRequestBody{TokenizedRequest: &fwkrh.TokenizedRequest{
			Prompts: []fwkrh.PromptTokens{{TokenIDs: make([]uint32, 3*testBlockSize)}},
		}},
	}
...
func TestProduceThenPreRequest_RepairsSelectedPrefill(t *testing.T) {
	...
	idx := &fakeKVCacheIndexer{
		...
		matchBlockKeys: func(_ context.Context, _ []kvblock.BlockHash, _ sets.Set[string]) (map[string]kvcache.PodMatch, error) {
			return map[string]kvcache.PodMatch{prefill: {MatchedBlocks: 16, ConfirmedBlocks: 16, WeightedScore: 16, BlocksByTier: map[string]int{"gpu": 16}}}, nil
		},
	}
	p := newProducerWithIndexer(ctx, idx)
	p.fullReportRepair = newTestRepair(0, prefill)
	...
	req := &scheduling.InferenceRequest{
		...
		Body: &fwkrh.InferenceRequestBody{
			Payload: payload,
			TokenizedRequest: &fwkrh.TokenizedRequest{
				Prompts: []fwkrh.PromptTokens{{TokenIDs: make([]uint32, len(keys)*testBlockSize)}},
			},
		},
	}
