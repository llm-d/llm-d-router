...
// Package kvcachehintv2 provides a PreRequest plugin that annotates requests
// from multi-turn sessions with bounded KV-cache retention hints for backends
// implementing the vLLM Context-Aware KV-Cache Retention API
// (https://github.com/vllm-project/vllm/pull/38514).
//
// The kv-cache-hint plugin protects a session's entire context. This plugin
// bounds the protection in two ways:
//
//   - Per session, the directive covers only the context head
//     [0, maxProtectedPrefixTokens): the system prompt and the shared history
//     head, which later turns and spawned subagents re-read, rather than the
//     full context.
//   - Globally, a protection budget caps the sum of live protected tokens
//     across all retention scopes. When a new protection would exceed the
//     budget, the request passes through unprotected and falls back to LRU.
//
// Both bounds exist because unbounded whole-context directives at high
// session concurrency pin more tokens than the backend KV pool holds, which
// degrades eviction to non-recency order and reduces the cache hit rate.
...
const (
	...
	// bytesPerTokenEstimate approximates prompt tokens from the raw body size
	// when the request carries no tokenization.
...
// RetainConfig sets one queue's retention policy: how long the session's
// protected head is shielded from eviction after a turn completes. Durations
// are time.ParseDuration strings, e.g. "30s". Unset fields inherit
// DefaultRetain.
...
type Config struct {
	...
	// MaxProtectedPrefixTokens bounds the protected range per session: the
	// directive covers [0, min(prompt tokens, this)). Must be > 0.
	MaxProtectedPrefixTokens int64 `json:"maxProtectedPrefixTokens,omitempty"`
	// ProtectionBudgetTokens caps the sum of live protected tokens across all
	// retention scopes. Sessions that would exceed it get no directive until
	// earlier protections expire. Size it below the backend KV pool so
	// unprotected blocks always leave the evictor room to work with. Must be
	// >= maxProtectedPrefixTokens.
	ProtectionBudgetTokens int64 `json:"protectionBudgetTokens,omitempty"`
}
...
var DefaultConfig = Config{
	...
	ProtectionBudgetTokens:   1_000_000,
}
...
type resolvedConfig struct {
	...
	maxProtectedPrefixTokens int64
	protectionBudgetTokens   int64
}
...
func (c Config) resolve() (resolvedConfig, error) {
	out := resolvedConfig{
		...
		maxProtectedPrefixTokens: c.MaxProtectedPrefixTokens,
		protectionBudgetTokens:   c.ProtectionBudgetTokens,
	}

	if out.maxProtectedPrefixTokens <= 0 {
		return out, fmt.Errorf("maxProtectedPrefixTokens must be > 0, got %d", out.maxProtectedPrefixTokens)
	}
	if out.protectionBudgetTokens < out.maxProtectedPrefixTokens {
		return out, fmt.Errorf("protectionBudgetTokens (%d) must be >= maxProtectedPrefixTokens (%d)",
			out.protectionBudgetTokens, out.maxProtectedPrefixTokens)
	}
...
// Plugin annotates session requests with bounded retention hints.
type Plugin struct {
...
func NewPlugin(name string, cfg Config) (*Plugin, error) {
	...
	return &Plugin{
		...
		cfg:       resolved,
		budget:    newBudgetTracker(resolved.protectionBudgetTokens),
	}, nil
}
...
// PreRequest writes a bounded retention directive into the request body.
// Requests without a session identifier, without an inter-turn prediction, or
// whose prediction matches no configured queue pass through untouched, as do
// requests whose protection would exceed the budget.
//
...
func (p *Plugin) PreRequest(ctx context.Context, request *fwksched.InferenceRequest, _ *fwksched.SchedulingResult) error {
	...
	protect := p.protectedTokens(request.Body)
	charged, ok := p.budget.reserve(scope, protect, duration)
	if !ok {
		if debugLogger := log.FromContext(ctx).V(logutil.DEBUG); debugLogger.Enabled() {
			used, budget := p.budget.usage()
			debugLogger.Info("kv-cache hint skipped: protection budget exhausted",
				"sessionID", string(sessionID), "scope", scope,
				"requestedTokens", protect, "usedTokens", used, "budgetTokens", budget)
		}
		return nil
	}
	...
	if debugLogger := log.FromContext(ctx).V(logutil.DEBUG); debugLogger.Enabled() {
		...
		debugLogger.Info("kv-cache hint set", "sessionID", string(sessionID),
			...
			"endTokens", charged, "usedTokens", used, "budgetTokens", budget,
...
// protectedTokens is the size of the context head the directive covers: the
// prompt token count when known (tokenized prompt, else a raw-body estimate),
// bounded by maxProtectedPrefixTokens. An unknown count charges the full
// bound so the budget never undercounts.
func (p *Plugin) protectedTokens(body *fwkrh.InferenceRequestBody) int64 {
	var tokens int64
	if body.TokenizedRequest != nil {
		tokens = int64(body.TokenizedRequest.TokenCount())
	}
	if tokens == 0 {
		tokens = int64(len(body.RawBody) / bytesPerTokenEstimate)
	}
	if tokens == 0 || tokens > p.cfg.maxProtectedPrefixTokens {
		return p.cfg.maxProtectedPrefixTokens
	}
	return tokens
}
