...
func TestDetector_SaturationIgnorePolicy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Single stale pod scores fresh average",
			pods: []fwkdl.Endpoint{
				makePodMetric("pod1", 1, 0.1, baseTime.Add(-2*time.Hour)),
			},
			wantSaturation: 0.0,
...
func TestDetector_StaleEndpointObservabilityIgnore(t *testing.T) {
	...
	for _, family := range families {
		if family.GetName() != "llm_d_epp_flow_control_stale_endpoints" {
			continue
		}
		for _, metric := range family.GetMetric() {
			for _, label := range metric.GetLabel() {
				if label.GetName() == "detector" && label.GetValue() == detectorName {
	...
	t.Fatalf("stale endpoint gauge series not found for detector %q", detectorName)
}
