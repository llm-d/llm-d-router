...
*Note: Endpoints with missing or stale metrics are scored according to `stalenessPolicy`: as 100% saturated under `saturated` (default), or excluded from the pool average under `ignore`.*
...
overload: a server too busy to serve its metrics endpoint is often the one that is saturated, so ignoring
would pass traffic to exactly the wrong endpoints. Deployments whose availability budget cannot absorb a dispatch halt
