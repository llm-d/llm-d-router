...
func TestVTC_EstimateInputTokens_Cascade(t *testing.T) {
	...
	t.Run("uses InputTokenCountHint when available", func(t *testing.T) {
		t.Parallel()
		ir := &fwksched.InferenceRequest{
			Body: &fwkrh.InferenceRequestBody{
				Completions: &fwkrh.CompletionsRequest{
					Prompt: fwkrh.Prompt{TokenIDs: make([]uint32, 200)}, // TokenCountHint returns 200
				},
			},
		}
