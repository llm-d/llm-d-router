...
//
// Priority order:
//  1. Client-provided hint via InputTokenCountHint() (e.g., pre-tokenized completions or /inference/v1/generate token IDs).
//  2. Length of a pre-tokenized prompt in InferenceRequestBody.TokenizedPrompt.
//  3. Character-based estimate: PromptText length / 4.
...
func estimateInputTokens(req flowcontrol.FlowControlRequest) float64 {
	...
	if ir != nil && ir.Body != nil {
		if hint := ir.Body.InputTokenCountHint(); hint > 0 {
			return float64(hint)
		}

		if ir.Body.TokenizedPrompt != nil && len(ir.Body.TokenizedPrompt.TokenIDs) > 0 {
			return float64(len(ir.Body.TokenizedPrompt.TokenIDs))
		}

		if text := ir.Body.PromptText(); len(text) > 0 {
