...
func TestRunningRequestsSizeScorer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			endpoints: []fwksched.Endpoint{
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 10}), nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 5}), nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 0}), nil),
			},
...
func TestRunningRequestsSizeScorerOmitsNeverScraped(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			endpoints: []fwksched.Endpoint{
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 1}), nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, &fwkdl.Metrics{}, nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 9}), nil),
			},
			want: []*float64{score(1.0), nil, score(0.0)},
		},
		{
			name: "equal observed sizes stay neutral when a never-scraped endpoint is present",
			endpoints: []fwksched.Endpoint{
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 5}), nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, &fwkdl.Metrics{}, nil),
				fwksched.NewEndpoint(&fwkdl.EndpointMetadata{}, scraped(fwkdl.Metrics{RunningRequestsSize: 5}), nil),
