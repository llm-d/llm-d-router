...
| `missThreshold` | int | `3` | Number of consecutive requests a session's bound pod may be absent from the candidate set before the session migrates. `session_id_header` only. |
