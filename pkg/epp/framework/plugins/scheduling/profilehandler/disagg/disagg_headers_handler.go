...
func (p *HeadersHandler) PreRequest(ctx context.Context, request *scheduling.InferenceRequest, schedulingResult *scheduling.SchedulingResult) error {
	...
	if request == nil {
		span.SetAttributes(
			telemetry.LLMDEPPPDDisaggregationUsed(false),
			telemetry.LLMDEPPEncodeDisaggregationUsed(false),
			telemetry.LLMDEPPDisaggReason("request_is_nil"),
		)
		return nil
	}
	if schedulingResult == nil {
		span.SetAttributes(
			telemetry.LLMDEPPPDDisaggregationUsed(false),
			telemetry.LLMDEPPEncodeDisaggregationUsed(false),
			telemetry.LLMDEPPDisaggReason("scheduling_result_is_nil"),
		)
		return nil
	}
	...
	case prefillProfileRunResult == nil:
		span.SetAttributes(
			telemetry.LLMDEPPPDDisaggregationUsed(false),
			telemetry.LLMDEPPPDReason("no_prefill_profile_result"),
		)
	case len(prefillProfileRunResult.TargetEndpoints) == 0:
		span.SetAttributes(
			telemetry.LLMDEPPPDDisaggregationUsed(false),
			telemetry.LLMDEPPPDReason("no_prefill_profile_target_endpoints"),
		)
	default:
		...
		span.SetAttributes(
			telemetry.LLMDEPPPDDisaggregationUsed(true),
			telemetry.LLMDEPPPDPrefillPodAddress(targetPod.Address),
			telemetry.LLMDEPPPDPrefillPodPort(targetPod.Port),
		)
	}
	...
	if encodeProfileRunResult == nil {
		span.SetAttributes(
			telemetry.LLMDEPPEncodeDisaggregationUsed(false),
			telemetry.LLMDEPPEncodeReason("no_encode_profile_result"),
		)
	...
	if len(encodeHostPorts) == 0 {
		span.SetAttributes(
			telemetry.LLMDEPPEncodeDisaggregationUsed(false),
			telemetry.LLMDEPPEncodeReason("no_encode_profile_target_endpoints"),
		)
	...
	span.SetAttributes(
		telemetry.LLMDEPPEncodeDisaggregationUsed(true),
		telemetry.LLMDEPPEncodeEndpoints(strings.Join(encodeHostPorts, ",")),
	)
	return nil
}
