...
func (h *PdProfileHandler) Pick(ctx context.Context, request *scheduling.InferenceRequest, profiles map[string]scheduling.SchedulerProfile,
	...
	span.SetAttributes(
		semconv.LLMDEPPProfileHandlerTotalProfiles(len(profiles)),
		semconv.LLMDEPPProfileHandlerExecutedProfiles(len(profileResults)),
	)
	...
	if _, executed := profileResults[h.decodeProfile]; !executed {
		...
		span.SetAttributes(
			semconv.LLMDEPPProfileHandlerDecision("run_decode"),
			semconv.LLMDEPPProfileHandlerSelectedProfile(h.decodeProfile),
		)
	...
	if h.decider != nil && h.decider.disaggregate(ctx, request, profileResults[h.decodeProfile].TargetEndpoints[0]) {
		...
		span.SetAttributes(
			semconv.LLMDEPPProfileHandlerDecision("prefill_decode"),
			semconv.LLMDEPPProfileHandlerSelectedProfile(h.prefillProfile),
		)
		return map[string]scheduling.SchedulerProfile{
	...
	span.SetAttributes(
		semconv.LLMDEPPProfileHandlerDecision("decode_only"),
	)
